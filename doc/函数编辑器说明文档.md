...
├── BaseFunctionInput (表达式输入组件，用于条件表达式编辑)
...
- **BaseFunctionInput**: 表达式输入组件，用于条件表达式编辑，基于Monaco Editor，支持全屏编辑
...
### 4. BaseFunctionInput组件特性

#### 4.1 组件概述
**位置**: `src/components/base/BaseFunctionInput.vue`

BaseFunctionInput是一个基于Monaco Editor的表达式输入组件，主要用于工作流中条件节点的表达式编辑。该组件提供了类似Element Plus textarea的外观和交互体验。

#### 4.2 核心功能
- **Monaco Editor集成**: 基于Monaco Editor提供代码编辑能力
- **样式模拟**: 容器样式完全模拟el-input的textarea样式
- **全屏编辑**: 支持全屏对话框模式编辑表达式
- **实时同步**: 编辑内容实时同步到v-model绑定的值
- **禁用状态**: 支持禁用状态，显示禁用样式

#### 4.3 编辑器配置
**位置**: `src/utils/MonacoEditor.ts` - `getDefaultMonacoEditorConfig()`

**默认配置**:
- **语言**: TypeScript（用于表达式编辑）
- **主题**: vs（浅色主题）
- **行号**: 关闭（lineNumbers: 'off'），左侧margin区域宽度为0
- **字体大小**: 12px（普通模式），14px（全屏模式）
- **自动布局**: 启用，自动调整编辑器尺寸
- **代码折叠**: 启用，自动折叠策略
- **自动换行**: 启用（wordWrap: 'on'）
- **代码提示**: 启用快速建议和代码补全
...
#### 4.4 样式设计
**容器样式**:
- **边框**: 1px实线，使用Element Plus的边框颜色变量
- **圆角**: 4px，使用Element Plus的圆角变量
- **背景**: 白色，使用Element Plus的背景色变量
- **内边距**: 5px 11px，与textarea保持一致
- **聚焦状态**: 聚焦时边框变为主题色
- **禁用状态**: 禁用时显示灰色背景和边框

**Monaco Editor样式优化**:
- 移除Monaco Editor的默认边框和圆角
- 左侧margin区域宽度设为0，不显示行号
- 编辑器内容区域从左侧开始，无额外边距
- 确保编辑器样式与容器样式完美融合

#### 4.5 全屏编辑功能
- **对话框模式**: 点击图标打开全屏编辑对话框
- **对话框尺寸**: 宽度840px，高度600px
- **对话框特性**: 
  - 不可通过点击遮罩关闭
  - 不可通过ESC键关闭
  - 关闭时销毁内容
- **操作按钮**: 提供"确定"和"取消"按钮
- **数据同步**: 全屏编辑的内容在确定后同步到主编辑器
...
#### 4.7 使用场景
- **条件节点表达式**: 用于编辑条件判断节点的Lua表达式
- **计算节点表达式**: 用于编辑计算节点的表达式
- **其他表达式输入**: 任何需要表达式编辑的场景

#### 4.8 技术实现细节
- **Monaco Manager**: 使用MonacoManager单例管理编辑器实例
- **实例复用**: 支持编辑器实例的创建和销毁
- **响应式绑定**: 使用useVModel实现双向数据绑定
- **生命周期管理**: 组件卸载时自动清理编辑器实例
...
### v3.1.0 (当前版本)
- **BaseFunctionInput组件优化**
  - 容器样式完全模拟el-input textarea，使用Element Plus CSS变量
  - 优化Monaco Editor配置，关闭行号显示（lineNumbers: 'off'）
  - 左侧margin区域宽度设为0，优化空间利用
  - 改进全屏编辑对话框体验
  - 支持聚焦和禁用状态的样式切换
  - 优化Monaco Editor与容器的样式融合
