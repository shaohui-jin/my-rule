...
调用 `generate(workflow, funcData)`：内部使用 `WorkflowAnalyzer` 做结构分析，再生成 **JavaScript 语法** 字符串（入口形如 `const doWork = (root, context) => { ... }`）。字段仍常命名为 `lua` 属历史原因，详见 [工作流脚本生成说明](工作流代码生成器文档.md)。
...
- [工作流脚本生成说明](工作流代码生成器文档.md)（`WorkflowAnalyzer` + `JsCodeParser`）
