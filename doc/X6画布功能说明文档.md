...
- **CustomNode.ts**: 自定义节点配置生成器
- **NodeFactory.ts**: 节点工厂，支持多种节点类型创建
- **WorkflowValidator.ts**: 工作流验证器
- **IteratorNode.ts**: 迭代器节点实现
- **GroupNode.ts**: 分组节点实现
- **DecisionTableEditPanelNode.vue**: 决策表编辑面板
...
- **聚合节点** (aggregate): 支持多个输入端口，合并数据并去重
- **迭代器节点** (iterator): 支持循环处理数据集合
- **决策表节点** (decision_tables_function): 支持复杂的决策表逻辑
- **外部数据表节点** (external_data_table): 支持外部数据源配置
- **自定义函数节点** (custom_function): 支持用户自定义Lua代码
...
### 节点外观设计
**位置**: `src/utils/workflow/CustomNode.ts`
...
### 端口布局算法
**位置**: `src/utils/workflow/CustomNode.ts` - `getPortPosition()` 函数
...
### 桩点生成逻辑
**位置**: `src/utils/workflow/CustomNode.ts` - `generatePorts()` 函数
...
### 迭代器管理
**位置**: `src/utils/workflow/IteratorManager.ts`
...
### 决策表编辑
**位置**: `src/components/workflow/panels/DecisionTableEditPanelNode.vue`
...
- outputData每项的conditionCheck不能为空（最后一个分支除外）
...
interface OutputData {
  ...
  type: string              // 参数类型
  subType?: string          // 参数子类型
  conditionCheck?: string   // 条件检查（条件节点专用）
  portId?: string           // 唯一端口ID
  widgetType?: string       // 控件类型
  defaultValue?: any        // 默认值
  options?: any[]           // 选项列表
...
- `node:edit_decisionTables`: 决策表编辑
