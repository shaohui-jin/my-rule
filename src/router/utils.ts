...
import { sessionKey, appendRoles, type DataInfo } from '@/utils/auth'
  ...
  if (getConfig()?.CachingAsyncRoutes) {
    ...
    return new Promise(resolve => {
      getMenuConfig().then(data => {
        usePermissionStoreHook().setConfigMenus(data)
        handleAsyncRoutes(cloneDeep([]))
        resolve(router)
      })
      // getAsyncRoutes().then(({ data }) => {
      //   appendRoles(data?.roles)
      //   rewriteRouteAuth(data?.routes)
      //   handleAsyncRoutes(cloneDeep([]))
      //   resolve(router)
      // })
    })
...
function rewriteRouteAuth(arrRoutes: Array<any>) {
  const children = router.options.routes[0].children
  for (const item of children) {
    for (const v of arrRoutes) {
      if (item.meta && item.meta.roles && item.meta?.roles[0] == v.role) {
        item.meta.auths = v.auths
      }
    }
  }
  return true
}
function getPathToList(routers: any[]) {
  const list = []
  const getPath = (routers: any[]) => {
...
function getFirstPath(routers: any[]) {
  const list = []
  const getPath = (routers: any[]) => {
    for (const item of routers) {
      if (item.children && item.children.length) {
