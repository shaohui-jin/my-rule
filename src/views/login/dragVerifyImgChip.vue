  ...
  <div class="drag-verify-container">
    <div :style="dragVerifyImgStyle">
      ...
      <div class="refresh" v-if="showRefresh && !isPassing">
        <i :class="refreshIcon" @click="refreshimg" />
      </div>
...
export default {
  ...
  computed: {
    handlerStyle: function () {
      return {
        ...
        background: this.isPassVerify ? this.successBg : this.handlerBg
      };
    },
    message: function () {
  ...
  data() {
    return {
      ...
      clipBarx: 0,
      showErrorTip: false,
      isPassVerify: false // 改样式用
    };
  },
  methods: {
    draw: function (ctx, x, y, operation) {
      const l = this.barWidth;
    ...
    checkimgLoaded: function () {
      ...
      const imgHeight = this.$refs.checkImg.height;
      const imgWidth = this.$refs.checkImg.width;
      ...
      const canvasCtx = this.$refs.maincanvas.getContext("2d");
      this.draw(canvasCtx, x, y, "fill");
      this.clipBarx = x;
      const moveCanvas = this.$refs.movecanvas;
      ...
      const moveCtx = this.$refs.movecanvas.getContext("2d");
    ...
    passVerify: function () {
      ...
      handler.style.background = this.successBg;
      console.log("xxx", this.successBg, handler.style.background);
