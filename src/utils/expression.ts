...
function filterEmptyTree(tree) {
  ...
  const filterTree = (node) => {
    if (!hasChild(node)) {
      ...
      node.children = node.children.filter(child => child.checked).map(child => {
  ...
  tree = tree.filter(node => node.checked).map(node => {
    delete node.checked
    return node
  });
  return tree;
...
export const getLuaCodeMapByExpression = async (nodeList: WorkflowNode[]) => {
  ...
  nodeList.forEach((node: WorkflowNode) => {
    ...
    if (isIfElseNode(node) && (node.version && node.version > '1.0.0')) {
      ...
      node.outputData
        .forEach((e, i, arr) => {
          funcArr.push({ source: e.conditionCheck, portId: e.portId, nodeId: node.id, nodeTitle: node.title })
          expressionParam.children.push({ id: e.portId, level: 2, code: '', name: '出参表达式', children: [] })
        })
      expressionParamArr.push(expressionParam)
    } else {
  ...
  let res = await batchGetLuaByExpressions(funcArr.map(e => {
    if (e.source) {
      funcIdArr.push(e.nodeId + '_' + e.portId)
    }
  ...
  res.forEach((_res, _index) => {
    ...
    expressionParamArr.find(e => e.id === id).children.forEach(e => {
      if (id + '_' + e.id === funcIdArr[_index]) {
        e.children.push(..._res.variables?.map(_e => {
          if (_e.includes('.')) {
            return {
