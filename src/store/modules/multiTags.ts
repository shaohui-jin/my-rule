...
export const useMultiTagsStore = defineStore({
  ...
  state: () => ({
    // 存储标签页信息（路由信息）
    multiTags: storageLocal().getItem<StorageConfigs>('sub-user-responsive-configure')
      ?.multiTagsCache
      ? storageLocal().getItem<StorageConfigs>('sub-user-responsive-tags')
      : [...routerArrays],
    multiTagsCache: storageLocal().getItem<StorageConfigs>('sub-user-responsive-configure')
      ?.multiTagsCache
  }),
  getters: {
    getMultiTagsCache() {
  ...
  actions: {
    multiTagsCacheChange(multiTagsCache: boolean) {
      ...
      if (multiTagsCache) {
        storageLocal().setItem('sub-user-responsive-tags', this.multiTags)
      } else {
        storageLocal().removeItem('sub-user-responsive-tags')
      }
    },
    tagsCache(multiTags) {
      this.getMultiTagsCache && storageLocal().setItem('sub-user-responsive-tags', multiTags)
    },
